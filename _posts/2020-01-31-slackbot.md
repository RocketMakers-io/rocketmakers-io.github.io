...
슬랙봇을 만드는 여러가지 방법이 있지만 나는 event api를 등록하고 생성한 슬랙봇에 app_mention 권한을 주어 해당 앱이 불려졌을 때 event api로 해당 데이터가 오도록 개발하였다.
...
해당 기능을 개발하기 위해서는 내가 가장 익숙한 Django, Django RestFramework와 슬랙 event api를 받는 slackclient, slackeventsapi 사용하였다.
...
만약 그것까지 서술하게 되면 너무 Django글 같아 질 것 같기 때문에..
...
해당 앱에는 출근시간, 예상 퇴근 시간, 실제 퇴근 시간등의 정보를 담은 model을 하나 생성했다.
...
views.py 파일에는 해당 이벤트를 받을 수 있는 api를 생성하였다. api는 post method만을 받을 수 있기 때문에 method는 post인 api를 생성해야한다!
...
chat_postMessage안에는 여러 parameter들이 들어갈 수 있다. 하지만 나는 메시지를 보내는 기능만 사용할 것이기 때문에 위 처럼 channel과 text만을 이용했다.

slack api의 request에 대해 설명해 놓은 것은 [slack event api](https://api.slack.com/events-api) 에 잘 설명되어있다.

일단 슬랙 event api를 등록하는데 슬랙 해당 api로 request를 보내 해당 api가 잘 작동되는지 테스트하는 부분이 있다.
...
이렇게 구현한다면 아마 event api validation은 통과될 것이다. 그렇다면 이제 슬랙을 커스텀 해보자.

나는 일단 슬랙 메시지를 가져오는 부분은 modules에 빼놓았다.
...
그리고 명령어의 내용들을 차례로 조건문으로 걸었다.

하지만 사용자가 내역 이라고 검색했다고 해서 내역만 내려오지 않아 해당 텍스트에 내역이라는 단어가 있는지 검색했다.

다행히도(?) 우리 멤버들중에는 이 슬랙봇을 그것과 관련해서 테스트해보는 사람은 없었지만, 단어가 들어있는 것만으로 실행이되면 치명적인 문제가 생길 수 있으니 해당 사항은 주의 바란다.

그리고 위와 같이 manager를 통하여 세부 기능들을 구현했다.

manager를 보기 전에 Util파일부터 뜯어보자

Django의 timezone utc를 설정하면 해당 모델에서 날짜와 시간을 뽑았을 때의 기본은 utc가 된다. 따라서 해당 날짜를 timezone aware형식으로 변경해야하기 때문에 Util에는 해당 기능들을 넣어놓았다.
...
그리고 Manager에는 이를 이용하여 출근과 퇴근시간을 체코에 맞춰 메시지에 담아 보내고 있다.
...
하지만 한명이 출근을 했다는 사실을 까먹고 다시 명령어를 입력하였는 경우를 생각하지 않는다면 하루에 한명이 여러번 출근을 하는 불상사가 발생하기 때문에 만약 존재한다면 생성하지 않고 이미 있는 데이터를 보여주어야한다.
...
여기까지 만들면 너무 서운하니까 출근 내역을 보여주는 기능도 하나 만들었다.
...
하지만 인간의 욕심은 끝이 없고 실수를 반복한다.

나는 점심 시간을 알려주고 퇴근 시간이 지났는데 퇴근을 하지 않은 사람들에게 메시지를 보내고 싶었다.

출근 시간은 자유로워도 퇴근 시간은 엄격해야(?) 했기에..

해당 기능을 생성하려면 crontab이 필요했다. `django_crontab` 을 이용하여 크론탭을 생성해보자.

일단 점심 시간 알림 함수를 생성하자.
...
이건 점심시간 보다 조금 복잡한 작업이다. 일단 각자 퇴근 시간이 지났는지 확인해야했고, 퇴근 시간이 지났다고 해서 1분 마다 해당 사용자에게 퇴근하라고 알림을 보내면 자칫 짜증나는 슬랙봇이 될 수 있다.

> 아니 버그가 안고쳐졌는데 어떻게 퇴근을 하라고

해당 상황을 방지하기 위해 한번 알림이 보내지면 보내지 않는 기능을 추가해야했다.

위의 model의 is_notify를 이용해서 해당 기능을 구현했다.
...
내가 주체가 되어 하는 개발들은 자의로 변경하고 강압이 없기 때문에 항상 재미있는 것 같다.

물론 일에서는 그렇지 않지만 이렇게 소소한 재미도 있어야하지 않을까.

이렇게 슬랙봇 생성이 완료되었다! 

조금씩 기능을 추가하고 만들어 나가야하겠지만 출퇴근은 이제 걱정없다!
